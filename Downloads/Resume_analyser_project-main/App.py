...
def run():
    ...
    img = Image.open('./Logo/projectIcon1.jpg')
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            # R_Data = resumeparse.read_file(save_image_path)
            # if resume_data['name'] and resume_data['email'] and resume_data['skills']:
            #     st.markdown('''<h4 style='text-align: left; color: #d73b5c;'>You are looking Fresher.</h4>''',unsafe_allow_html=True)
            # else:
            #     st.markdown('''<h4 style='text-align: left; color: #d73b5c;'>You are hello.</h4>''',unsafe_allow_html=True)
            ...
            def clean_resume(resume_text):
                ...
                return clean_text
            # try:
            #     resume_byte = resume_data['skills']
            #     resume_bytes = " ".join(resume_byte)
            ...
            try:
                resume_byte = resume_data['skills']
                if isinstance(resume_byte, str):
                    # No need to decode if it's already a string
                    resume_text = resume_byte
                elif isinstance(resume_byte, list):
                    # Join the list of skills into a single string
                    resume_text = " ".join(resume_byte)
                else:
                    # Handle unexpected data type
                    st.error("Unexpected data type for skills.")
                ...
                st.error("Skills data not found in resume.")
                st.stop()  # Stop execution
            ...
            if resume_data['name'] and resume_data['email'] and resume_data['skills']:
                ...
                if 'hobbies' or 'interests' in resume_text.lower():
                    resume_score = resume_score + 20
                    st.markdown(
                ...
                # Inside the resume analysis section
        ...
        if st.button('Login'):
            if ad_user == 'nexus' and ad_password == 'nexus':
